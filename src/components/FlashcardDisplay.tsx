...
import { Volume2, VolumeX, Trash } from 'lucide-react';
...
export const FlashcardDisplay = ({
  ...
  const handleReadAloud = () => {
    if (isReading) {
      ...
      const femaleVoice = voices.find(voice => voice.name.toLowerCase().includes('female') || voice.name.toLowerCase().includes('zira') || voice.name.toLowerCase().includes('hazel') || voice.name.toLowerCase().includes('susan') || voice.name.toLowerCase().includes('samantha') || voice.name.toLowerCase().includes('karen') || voice.name.toLowerCase().includes('moira') || voice.name.toLowerCase().includes('tessa') || voice.name.toLowerCase().includes('veena') || voice.name.toLowerCase().includes('fiona') || voice.name.toLowerCase().includes('kate') || voice.name.toLowerCase().includes('serena')) || voices.find(voice => voice.lang.startsWith('en')) || voices[0];
      if (femaleVoice) {
  ...
  const handleDelete = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (onDelete && confirm('Are you sure you want to delete this flashcard?')) {
      onDelete();
    }
  };
    ...
    <div className="relative">
      {onDelete && (
        <Button
          ...
          variant="outline"
          size="sm"
          className="absolute -top-2 -right-2 z-10 w-8 h-8 rounded-full bg-red-50 hover:bg-red-100 border-red-200 text-red-600"
        >
          <Trash size={14} />
        </Button>
      )}
